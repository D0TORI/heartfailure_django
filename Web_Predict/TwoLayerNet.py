...
class TwoLayerNet:
    ...
    def copy_layers(self):
        return {name: layer.copy() for name, layer in self.layers.items()}

    def update_best_state(self, val_accuracy):
        if val_accuracy > self.best_val_accuracy:
            ...
            self.best_params = {
                ...
                'W3': np.copy(self.params['W3']),
                'b3': np.copy(self.params['b3']),
                'W4': np.copy(self.params['W4']),
                'b4': np.copy(self.params['b4']),
            }
            self.best_layers = self.copy_layers()
    
    def save_best_model(self, file_path='best_model.npz'):
        if self.best_params is not None and self.best_layers is not None:
            model_state = {
                'params': self.best_params,
                'layers': self.best_layers,
            }
            np.savez(file_path, **model_state)

    def load_model(self, file_path='model.npz'):
        if os.path.exists(file_path):
            model_state = np.load(file_path, allow_pickle=True)
            self.params = model_state['params'].item()
            self.layers = model_state['layers'].item()
        else:
